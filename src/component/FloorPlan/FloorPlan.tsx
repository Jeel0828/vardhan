...
export default function FloorPlan() {
  ...
  const getPosition = (index: number) => {
    ...
    if (diff < -floorPlans.length / 2) diff += floorPlans.length;
    if (diff > floorPlans.length / 2) diff -= floorPlans.length;
    return diff;
  };
  ...
  return (
    <section
      className="w-full bg-[#fff] py-16 md:py-20 relative overflow-hidden"
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
        <h2
          className="text-3xl sm:text-4xl font-medium text-black mb-3 md:mb-4 tracking-tight"
          style={{ letterSpacing: "0.02em" }}
        >
          Project Specification, Configuration & Floor Plans
        </h2>

        <p className="text-lg sm:text-xl md:text-2xl text-black font-light mb-6">
          Discover Your Perfect Home Layout
        </p>

        <div className="relative min-h-[520px] sm:min-h-[580px] lg:min-h-[640px] flex items-center justify-center">
          <button
            onClick={goToPrevious}
            className="absolute left-2 cursor-pointer sm:left-6 lg:left-12 top-1/2 -translate-y-1/2 z-30 w-12 h-12 sm:w-14 sm:h-14 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:text-[#c7d35b] transition-all duration-300 shadow-xl"
          ...
          <button
            onClick={goToNext}
            className="absolute right-2 cursor-pointer sm:right-6 lg:right-12 top-1/2 -translate-y-1/2 z-30 w-12 h-12 sm:w-14 sm:h-14 bg-black/40 hover:bg-black/60 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:text-[#c7d35b] transition-all duration-300 shadow-xl"
            ...
            {floorPlans.map((plan, index) => {
              ...
              return (
                <div
                  ...
                  className="absolute transition-all duration-700 ease-out"
                  style={{
                    transform: `translateX(${offset}px) scale(${isCenter ? 1 : 0.82})`,
        ...
        <div className="mt-12">
          <button
            className="px-8 py-4 sm:px-10 sm:py-5 bg-[#c7d35b] hover:bg-[#b8c550] text-[#fff] font-normal text-base sm:text-lg rounded-full transition-all duration-300 shadow-lg hover:shadow-[#c7d35b]/50 hover:scale-105"
            style={{ letterSpacing: "0.02em" }}
          >
